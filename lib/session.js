 ...
 * Payload session: { typ: "session", playerId, email, nickname, exp }
 * Payload handoff: { typ: "handoff", playerId, email, nickname, iat, exp }
 *
 * Verify rejects session tokens: games must only accept short-lived handoff
 * tokens (typ === "handoff", iat required, TTL enforced).
 */
...
export function verifySessionToken(token) {
  ...
  // Reject handoff tokens presented as session cookies.
...
export function verifyHandoffToken(token) {
  ...
  // Purpose-bound: session cookies (typ "session" or legacy no-typ without iat)
  // must never pass as short-lived handoff tokens.
  if (payload.typ !== "handoff") {
    throw Object.assign(new Error("Invalid handoff token"), {
      status: 401,
      code: "INVALID_TOKEN",
    });
  ...
  const now = Math.floor(Date.now() / 1000);
  if (!payload.playerId || !payload.email || !payload.exp) {
    throw Object.assign(new Error("Invalid handoff token"), {
      status: 401,
      code: "INVALID_TOKEN",
    });
  ...
  // Enforce documented short TTL even if exp was crafted longer.
  if (now - iat > HANDOFF_TTL_SEC + 30) {
    throw Object.assign(new Error("Handoff token expired"), {
      status: 401,
      code: "TOKEN_EXPIRED",
    });
